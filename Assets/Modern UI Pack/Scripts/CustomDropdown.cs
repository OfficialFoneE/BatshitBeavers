...
namespace Michsky.UI.ModernUIPack
{
    public class CustomDropdown : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            //{
            //    //InstantiateDropdownItem(dropdownItems[i]);
            //    Debug.Log("INST ITEM");
        ...
        public void InstantiateDropdownItem(Item item)
        {
            ...
            setItemText = go.GetComponentInChildren<TextMeshProUGUI>();
            textHelper = item.itemName;
            setItemText.text = textHelper;

            //Transform goImage;
            //goImage = go.gameObject.transform.Find("Icon");
            //setItemImage = goImage.GetComponent<Image>();
            //imageHelper = item.itemIcon;

            //setItemImage.sprite = imageHelper;

            Button itemButton;
            itemButton = go.GetComponent<Button>();
            itemButton.onClick.AddListener(item.OnItemSelection.Invoke);

            int dropdownItemIndex = dropdownItems.Count;

            itemButton.onClick.AddListener(delegate { ChangeDropdownInfo(dropdownItemIndex); } );
            itemButton.onClick.AddListener(Animate);

            //if (invokeAtStart == true)
            //{
            //    item.OnItemSelection.Invoke();
            //}
